...
/**
 * Serializes a SearchParams object to URL query string params.
 * URL format: /results?origin=FCO&destination=LHR&dep=2025-03-15&ret=2025-03-22
...
export function serializeSearchParams(params: SearchParams): URLSearchParams {
  ...
  if (params.returnDate) {
    search.set('ret', params.returnDate);
  }
...
/**
 * Deserializes URL query string params back to a SearchParams object.
...
export function deserializeSearchParams(search: URLSearchParams): SearchParams | null {
  ...
  if (!origin || !destination || !dep || !type || adultsRaw === null || childrenRaw === null || infantsRaw === null || !cabin) {
    return null;
  }

  // Validate tripType
  if (!VALID_TRIP_TYPES.includes(type as (typeof VALID_TRIP_TYPES)[number])) {
    return null;
  }

  // Validate cabinClass
  if (!VALID_CABIN_CLASSES.includes(cabin as CabinClass)) {
    return null;
  }
