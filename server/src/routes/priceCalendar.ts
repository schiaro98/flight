 ...
 * Returns: array of { date, minPrice, currency, isLowest }
 */
router.get('/', async (req: Request, res: Response) => {
  ...
  const results: { date: string; minPrice: number | null; currency: string }[] = [];
  ...
  // Fill in past dates as null
  const allDays: { date: string; minPrice: number | null; currency: string }[] = [];
  for (let d = 1; d <= daysInMonth; d++) {
    const date = `${y}-${String(m).padStart(2, '0')}-${String(d).padStart(2, '0')}`;
    const found = results.find((r) => r.date === date);
    allDays.push(found ?? { date, minPrice: null, currency: 'EUR' });
  }

  // Compute isLowest
  const available = allDays.filter((e) => e.minPrice !== null).map((e) => e.minPrice as number);
  const avg = available.length > 0 ? available.reduce((a, b) => a + b, 0) / available.length : 0;

  const response = allDays.map((e) => ({
    ...e,
    isLowest: e.minPrice !== null && e.minPrice < avg,
  }));
